...
@Component({
    ...
    template: `
        <a class="flex items-center" href="#">
            ...
            <span class="text-surface-900 dark:text-surface-0 font-medium text-2xl leading-normal mr-20">SIPRODIG</span>
        </a>
        ...
        <div class="items-center bg-surface-0 dark:bg-surface-900 grow justify-between hidden lg:flex absolute lg:static w-full left-0 top-full px-12 lg:px-0 z-20 rounded-border">
            <ul class="list-none p-0 m-0 flex lg:items-center select-none flex-col lg:flex-row cursor-pointer gap-8">
                <li>
                    <a (click)="router.navigate(['/'])" pRipple class="px-0 py-4 text-surface-900 dark:text-surface-0 font-medium text-xl">
                        <span>Home</span>
                    </a>
                </li>
                <li>
                    <a (click)="router.navigate(['/product'])" pRipple class="px-0 py-4 text-surface-900 dark:text-surface-0 font-medium text-xl">
                        <span>Produk</span>
                    </a>
                </li>
                <li>
                    <a (click)="router.navigate(['/testimoni'])" pRipple class="px-0 py-4 text-surface-900 dark:text-surface-0 font-medium text-xl">
                        <span>Testimoni</span>
                    </a>
                </li>
                <li *ngIf="authService.isAuthCustomer()">
                    <a (click)="router.navigate(['/my-order'])" pRipple class="px-0 py-4 text-surface-900 dark:text-surface-0 font-medium text-xl">
                        <span>Pesanan</span>
                    </a>
                </li>
            </ul>
            @if (hasLoggedIn) {
                ...
                <div class="flex border-t lg:border-t-0 border-surface py-4 lg:py-0 mt-4 lg:mt-0 gap-2">
                    <button pButton pRipple label="Masuk" routerLink="/masuk" [rounded]="true" [text]="true"></button>
                    <button pButton pRipple label="Daftar" routerLink="/daftar" [rounded]="true"></button>
                </div>
            }
        </div>
...
export class TopbarWidget {
    ...
    constructor(
        ...
        if (token) {
            const decodedToken: any = authService.decodedToken(token);
            this.customer = utils.toTitleCase(decodedToken?.customerName);
        }
    }
